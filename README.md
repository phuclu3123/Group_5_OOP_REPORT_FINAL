...
1.  **Thành viên 1**: Phát triển lõi nghiệp vụ Actors (`Person`, `Staff`, `Admin`, `Driver`, `Customer`), Security và hệ thống tính lương (`ISalaryCalculable`).
...
3.  **Thành viên 3**: Phát triển lõi cơ sở hạ tầng (`Warehouse`, `WarehouseLocation`, `Equipment`, `Vehicle`, `Engine`, `MaintenanceLog`).
4.  **Thành viên 4**: Thiết kế và triển khai cơ chế lưu trữ dữ liệu an toàn `JsonRepository<T>` và Repository Factory.
5.  **Thành viên 5**: Xây dựng giao diện Windows Forms (`Logistics.WinFormsUI`), tích hợp phân quyền chức năng và viết kịch bản kiểm thử (`Logistics.SmokeTests`).
